...
3.  **STAFF (Nhân viên / Thợ may):**
    ...
    * **Quyền hạn:** Quyền hạn bị giới hạn. Chủ yếu là `READ` (Đọc) và `UPDATE` (Cập nhật) trạng thái cho các đơn hàng hoặc lịch hẹn *đã được gán* cho mình. Không có quyền tạo/xóa các tài nguyên quan trọng (như Sản phẩm, Vải...).
...
1.  **State Machine cho `Order` (Đơn hàng):**
    ...
    * *Quy tắc nghiệp vụ:* Không thể hủy đơn khi đã ở trạng thái `Shipping` hoặc `Completed`.
...
* `Staff` **phải** có khả năng `XEM` danh sách công việc (đơn hàng, lịch hẹn) *chỉ được gán cho mình*.
...
* Hệ thống **phải** đảm bảo **Transactional Integrity (Toàn vẹn Giao dịch)**.
    * *Ví dụ:* Khi tạo đơn hàng, việc ghi vào bảng `Order`, `OrderItem`, và `Payment` phải nằm trong một `Transaction`. Nếu một bước thất bại, toàn bộ phải được `Rollback`. (Sử dụng `Prisma Transaction`).
...
* *Lý do:* Thiết kế này cho phép hệ thống dễ dàng bảo trì và có khả năng "bóc tách" một module (ví dụ: `Appointments`) ra thành Microservice riêng trong tương lai nếu cần mở rộng.
